...
# Import log reader for parsing logs
...
class ExtractProcessManager:
    ...
    async def stop_parse(self) -> Dict[str, Any]:
        ...
        if not self.processes['parse'] or self.processes['parse'].poll() is not None:
            return {"status": "not_running"}
            
        try:
            self.processes['parse'].terminate()
            self.processes['parse'].wait(timeout=5)
            logger.info("Stopped Extract API parsing")
            return {"status": "stopped"}
        except subprocess.TimeoutExpired:
    ...
    async def _stream_process_logs(self, phase: str, process):
        ...
        try:
            if phase == 'parse':
                # For parsing phase, read from log files via log_reader
                await self._stream_file_logs(phase, process)
            else:
                # For RSS and Media phases, read from stdout (existing behavior)
    ...
    async def _stream_stdout_logs(self, phase: str, process):
        """Stream logs from process stdout (RSS, Media)"""
    ...
    async def _stream_file_logs(self, phase: str, process):
        """Stream logs from log files via log_reader (Parse)"""
        log_reader = get_log_reader()
        if not log_reader:
            logger.warning("Log reader not available, falling back to stdout")
            await self._stream_stdout_logs(phase, process)
            return
        
        logger.info(f"Starting file log streaming for {phase} phase")
        
        # Monitor both process status and log files
        while process.poll() is None:
            try:
                # Get log entry from log_reader
                log_entry = await log_reader.get_log_entry()
                if log_entry:
                    logger.debug(f"Log entry received: {log_entry}")
                    
                    # Filter for parsing-related logs
                    if (log_entry.get('log_file') == 'parsing' or 
                        'content_parser' in log_entry.get('logger', '') or
                        'services.content_parser' in log_entry.get('logger', '')):
                        
                        # Format log entry for WebSocket
                        log_data = {
                            "phase": phase,
                            "timestamp": log_entry.get('timestamp', datetime.now().isoformat()),
                            "message": log_entry.get('message', ''),
                            "level": log_entry.get('levelname', log_entry.get('level', 'info')).lower()
                        }
                        
                        logger.info(f"Sending {phase} log to WebSocket: {log_data['message']}")
                        await self._send_log_to_websockets(log_data)
                
                await asyncio.sleep(0.1)
                
            except Exception as e:
                logger.warning(f"Error reading logs for {phase}: {e}")
                await asyncio.sleep(1.0)
        
        logger.info(f"Process {phase} finished, stopping file log streaming")
