...
class Database:
    ...
    def article_exists(self, url: str) -> bool:
        ...
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT 1 FROM articles WHERE url = ? AND is_deleted = 0 LIMIT 1', (url,))
            return cursor.fetchone() is not None
    
    def soft_delete_article(self, article_id: str, deleted_by: str = 'dashboard') -> bool:
        """Soft delete an article by marking it as deleted"""
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute("""
        ...
        try:
            with self.get_connection() as conn:
                ...
                cursor.execute("""
                    ...
                    WHERE article_id = ? AND is_deleted = 1
                """, (article_id,))
                return cursor.rowcount > 0
        except Exception as e:
            logger.error(f"Error restoring article {article_id}: {e}")
            return False
    ...
    def get_pending_articles_count(self) -> int:
        """Count articles with 'pending' status (excluding deleted)"""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("""
                SELECT COUNT(*) as count FROM articles 
                WHERE content_status = 'pending' AND is_deleted = 0
            """)
    ...
    def get_pending_articles(self, limit: int = 10, source_id: Optional[str] = None) -> List[Dict]:
        """Get pending articles for parsing (excluding deleted)"""
        with self.get_connection() as conn:
            query = """
    ...
    def get_stats(self) -> Dict:
        ...
        with self.get_connection() as conn:
            ...
            # Articles by status (excluding deleted)
            cursor.execute("""
