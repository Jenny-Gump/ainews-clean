...
**Версия:** 4.2 (19 августа 2025)  
**Статус:** Production Ready + Memory Management + Stop Button Fix  
**Основа:** Анализ кода из change_tracking/monitor.py, database.py  
**Обновления:** Управление памятью, исправление кнопки Stop, улучшенные таймауты
...
- **PID файл** - использование для точной остановки процесса
- **Улучшенные pkill паттерны** - обработка путей с пробелами
- **exec true** - полное завершение bash процесса вместо exit 0
- **Таймаут 180s** - добавлен в scan_sources_sequential для каждого источника
