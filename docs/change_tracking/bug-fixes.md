...
## 🐛 Bug #6: Постоянные зависания RSS Discovery и Change Tracking

### Описание проблемы
**Дата обнаружения:** 17 августа 2025  
**Серьезность:** КРИТИЧЕСКАЯ  
**Статус:** ✅ ИСПРАВЛЕНО (MVP решение)

### Симптомы
- RSS Discovery и Change Tracking постоянно зависали на разных источниках
- Проблема "исправлялась" 10+ раз, но возвращалась
- Процессы требовали принудительного завершения
- Система могла зависнуть навсегда на одном источнике

### Техническая причина - МНОГОСЛОЙНЫЕ КОНФЛИКТУЮЩИЕ ТАЙМАУТЫ

**Архитектурная проблема:**
```
User Process → RSS Discovery → aiohttp → asyncio.wait_for → Firecrawl API
     ∞           30s           40s         45s              360s(!)
```

**Конкретные проблемы:**
1. **RSS Discovery**: 
   - aiohttp таймаут 30 сек, но `feedparser.parse()` - синхронный без таймаута!
   - Мог зависнуть навсегда на парсинге RSS

2. **Change Tracking**:
   - 3 попытки retry с exponential backoff (5s, 10s, 20s)
   - asyncio.wait_for(45s) оборачивал Firecrawl с таймаутом 40s
   - Искусственная задержка 8 секунд на каждый источник

3. **Firecrawl Client**:
   - Дефолтный таймаут 360 секунд (6 минут!)
   - _wait_for_job_completion мог ждать 6 минут
   - Конфликт с другими таймаутами

### Решение - MVP ПОДХОД "FAIL FAST, SKIP FORWARD"

#### 1. Process Supervisor (новый компонент)
**Файл:** `core/process_supervisor.py` (создан)
- Каждый источник в отдельном процессе
- Жёсткий таймаут 60 секунд с SIGKILL
- Никаких retry - ошибка = skip
- Полная изоляция от зависаний

#### 2. Упрощение RSS Discovery
**Файл:** `services/rss_discovery.py`
```python
# Было: синхронный feedparser без таймаута
feed = feedparser.parse(content)

# Стало: асинхронный с таймаутом 10 сек
feed = await asyncio.wait_for(
    asyncio.to_thread(feedparser.parse, content),
    timeout=10
)
```
- HTTP таймаут уменьшен: 30s → 20s
- Убрана обработка asyncio.TimeoutError

#### 3. Упрощение Change Tracking
**Файл:** `change_tracking/monitor.py`
- max_retries: 3 → 1 (по умолчанию без retry)
- Убран exponential backoff
- Убрана задержка sleep(8)
- Таймаут: 45s → 30s

### Результат исправления
- ✅ **RSS Discovery**: 2 секунды на источник (было зависание)
- ✅ **Change Tracking**: 10-11 секунд на источник
- ✅ **Process Supervisor**: 0 killed процессов в тестах
- ✅ **ГАРАНТИЯ**: Система НИКОГДА не зависнет >60 сек
...
# Результат:
✅ RSS Discovery завершен за 2.0 секунд
✅ Change Tracking за 10-11s на источник  
✅ Process Supervisor: killed=0
✅ ВСЕ ТЕСТЫ ПРОЙДЕНЫ УСПЕШНО
```

### Ключевые изменения философии
1. **НЕТ сложных retry** - fail = skip и дальше
2. **НЕТ вложенных таймаутов** - один уровень контроля
3. **НЕТ долгих ожиданий** - максимум 30 сек на операцию
4. **ЕСТЬ изоляция** - каждый источник в своём процессе

---

## 📊 Статистика исправлений (обновлено)
...
| 15.08.2025 | #4 | ВЫСОКАЯ | ✅ Исправлено | 1 час |
| 17.08.2025 | #5 | КРИТИЧЕСКАЯ | ✅ Исправлено | 15 минут |
| 17.08.2025 | #6 | КРИТИЧЕСКАЯ | ✅ Исправлено MVP | 1 час |
