...
def show_stats():
    ...
    try:
        ...
        stats_lines = ["\n📊 Статистика Extract API системы:", "=" * 50, "", "Статьи по статусам:"]
        for stat in articles_stats:
            stats_lines.append(f"   {stat['content_status']}: {stat['count']}")
        ...
        if media_stats:
            ...
            stats_lines.extend([
                "",
                "📸 Медиа-файлы:",
                f"   Всего: {media['total_media']}",
                f"   С метаданными: {media['with_metadata']}"
            ])
...
def signal_handler(signum, frame):
    ...
    logger.warning(f"⚠️ Получен сигнал {signum}, запрашиваем graceful shutdown...")
