...
✅ **🆕 Оптимизирована загрузка Articles** - по умолчанию показываются только pending статьи (82 вместо 931)

---

## 🆕 Обновления v2.11 (19 августа 2025)

### Оптимизация загрузки вкладки Articles
- **Проблема**: Медленная загрузка вкладки Articles из-за загрузки всех 931 статей и лишних запросов медиа-файлов
- **Решение**:
  - По умолчанию загружаются только pending статьи (82 вместо 931)
  - Удалены все запросы медиа-файлов (больше не используются в дашборде)
  - Добавлена опция "All Articles" в фильтре Status с предупреждением о медленной загрузке
  - Визуальная индикация активного фильтра
- **Результат**: Загрузка ускорилась в **11 раз**
- **Изменённые файлы**:
  - `monitoring/api/core_supabase.py` - удалены запросы медиа
  - `monitoring/api/articles.py` - pending по умолчанию
  - `monitoring/static/index.html` - улучшенный UI фильтров
